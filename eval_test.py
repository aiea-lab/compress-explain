...
# TESTS
vanilla_results = eval.grad_cam_batch_labels(model, vanilla_model, train_loader, comp_methods=['cosine', 'l2', 'iou'], target_layers_x=[model.network.layer4[2].conv2], target_layers_y=[vanilla_model.network.layer4[2].conv2], threshold=0.3)
f = open("results.txt", "a")
f.write(f"vanilla_results: {str(vanilla_results)}")
f.close()
lottery_results = eval.grad_cam_batch_labels(model, lottery_model, train_loader, comp_methods=['cosine', 'l2', 'iou'], target_layers_x=[model.network.layer4[2].conv2], target_layers_y=[lottery_model.network.layer4[2].conv2], threshold=0.3)
f = open("results.txt", "a")
f.write(f"lottery_results: {str(lottery_results)}")
f.close()
# gal_results = eval.grad_cam_batch_labels(model, gal_model, train_loader, comp_methods=['cosine', 'l2', 'iou'], target_layers_x=[model.network.layer4[2].conv2], target_layers_y=[gal_model.network.layer4[2].conv2], threshold=0.3)
hydra_results = eval.grad_cam_batch_labels(model, hydra_model, train_loader, comp_methods=['cosine', 'l2', 'iou'], target_layers_x=[model.network.layer4[2].conv2], target_layers_y=[hydra_model.network.layer4[2].conv2], threshold=0.3)
f = open("results.txt", "a")
f.write(f"hydra_results: {str(hydra_results)}")
f.close()
attribute_preserve_results = eval.grad_cam_batch_labels(model, attribute_preserve_model, train_loader, comp_methods=['cosine', 'l2', 'iou'], target_layers_x=[model.network.layer4[2].conv2], target_layers_y=[attribute_preserve_model.network.layer4[2].conv2], threshold=0.3)
f = open("results.txt", "a")
